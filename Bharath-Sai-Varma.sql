...
select sum(quantity) as totalproducts_sold from ECOMMERCE where PURCHASE_DATE >= '2019-02-01' and PURCHASE_DATE <'2019-03-01';

--2. write an sql query to total sales amount in each year ? 


SELECT SUBSTR(purchase_date, 1, 4) AS sales_year, SUM(quantity * price) AS totalsales_amount
FROM ECOMMERCE
...
sum(quantity*price) as total_sales from ECOMMERCE
...
SELECT DISTINCT(PRODUCTNAME) AS UNIQUEPRODUCT_NAME,SALES_YEAR 
FROM CTE
