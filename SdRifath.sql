...
SELECT SUBSTR(PURCHASE_DATE,1,4) as "YEAR" ,sum(PRICE*QUANTITY) as Amount from ECOMMERCE group by SUBSTR(PURCHASE_DATE,1,4); 
...
select COUNTRY,Count(Distinct(CUSTOMERNO)) as CUSTOMERS_COUNT from ECOMMERCE Group By COUNTRY order by COUNTRY asc ; 
...
WITH CTE AS (
    ...
    SELECT Distinct(PRODUCTNAME) AS UniqueProductName,SALES_YEAR FROM CTE GROUP BY SALES_YEAR, PRODUCTNAME ORDER BY SALES_YEAR asc;
